...
We can clearly see from the vertical lines that we have an oversegmentation in the third segment. 
The histogram and Q-Q plot both show some signs of deviations from a strict normal distribution, especially in the tails, which might be due to the presence of heteroscedasticity in the data.
...
hist(residuals(out)[, 1], main = "Histogram of the residuals")

# Generate a Q-Q plot for the residuals
qqnorm(residuals(out)[, 1], main = "Normal Q-Q Plot of the Residuals")
qqline(residuals(out)[, 1], col = "red")  # Adds a reference line


plot(fitted(out), residuals(out)[, 1], xlab = "fitted", ylab="residuals")
```
