...
1. Determine if the following processes are stationary, piecewise stationary, or non-stationary:
  ...
  a. $y_t = y_{t - 1} + \epsilon_t, \quad \ t = 2, \dots, n, y_1 = 0, \epsilon_{1:n} \sim N(0, 1)$. 
  Suggestion: try to compute the variance of the process. 
  
  b. $ y_t = \epsilon_t + 3 \cdot \mathbb{1}(t > 50), \ t = 1, \dots, 100, \quad \epsilon_{1:100} \sim N(0, 1) $
  
  c. $y_t = 0.05 \cdot t + \epsilon_t, \ t = 1, \dots, 100, \quad \epsilon_{1:100} \sim N(0, 1)$
    
2.  Compute the mean and variance of the absolute value of difference between the two pre-change and post-change sample means when there is no change, and show that when multiplied by the normalizing constant $\frac{1}{\sigma^2} \sqrt{\frac{\tau(n-\tau)}{n}}$ this follows a standard normal distribution.

## Lab 1 Exercises

1. Code the CUSUM algorithm for an unknown change location, based on the pseudocode above.

2. Compute the CUSUM statistics for all changepoint locations in the Simpsons data, plot the result over time. Check it against the plot above. 
