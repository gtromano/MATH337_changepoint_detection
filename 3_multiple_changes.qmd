...
$$
    \mathcal{L}(y_{s+1:t}) =  - 2  \min_\theta \sum_{i = s + 1}^{t} \log(f(y_i, \theta))
$${#eq-segment_cost}

with $f(y, \theta)$ being the likelihood for data point $y$ if the segment parameter is $\theta$.
...
$$
\sum_{i = s + 1}^{t} \log f(y_t | \theta) = \frac{t - s + 1}{2} \log(2\pi \sigma^2) -\frac{1}{2 \sigma^2} \sum_{i = s + 1}^{t} (y_t - \mu)^2.
$$

This is maximized for  $\bar{y}_{s+1:t} = \frac{1}{t - s} \sum_{i = s + 1}^{t} y_t$. Therefore, plugging this into our equation @eq-segment_cost, the cost of a segment will be given by:
