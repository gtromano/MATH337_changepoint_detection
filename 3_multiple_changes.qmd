...
Fortunately, this problem can be solved efficiently using a sequential, dynamic programming algorithm: **Optimal Partitioning (OP)**. OP solves @eq-pen-cost exactly through the following recursion.
...
set.seed(5)
x <- c(rnorm(6))
plot(x, type = "l", xlim = c(-1, 10), ylim = c(-12, 4), xlab= "t", ylab = " ", col = "grey")
...
**Step 2.** Now, at the second step, we can choose either to have the whole sequence in a second segment alone (again, $\beta$ cancels out with $Q_{0, \beta} = -\beta$), and this will be given by $\mathcal{L}(y_{1:2})$ (dotted line), or we have the optimal cost from step 1 (that had only the first point in its own segment), to which we have to sum the cost relative to a second segment $\mathcal{L}(y_{2:2})$ that puts the second point alone, and the penalty $\beta$ as we have added a new segment! We maximise across the two, and this gives us $Q_{2, \beta}$

```{r, echo=FALSE}
op_plot <- function(t){
  set.seed(5)
  x <- c(rnorm(6))
  plot(x, type = "l", xlim = c(-1, 10), ylim = c(-13, 4), xlab= "t", ylab = " ", col = "grey")
  points(1:6, x, pch = 19, cex = 1.5, col = "grey")
