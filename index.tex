...
with computations and details about the methodologies, and, finally,
during the lab sessions, you'll give a go at programming the various
algorithms and running real-world examples.
...
\textbf{Formal Notation.} In time series analysis, use an index \(t\) to
represent time or order on a given set of observations. The time series
...
Often, we are also interested in subsets of a time series, especially
when investigating specific ``windows'' or ``chunks'' of the data. A
subset of a time series, starting from time \(l\) to time \(u\), will be
...
Time series can have various statistical properties that explain how
they behave over time, and they can be characterized based on those. Let
us look at three examples of time series:
...
\item
  The leftmost time series, was generated by sampling random normal
  variables \(y_t = \epsilon_t, \ \epsilon_t \sim \mathcal{N}(0, 1)\).
  In this case: \[
      \mathbb{E}(y_t) = \mathbb{E}(\epsilon_t) = 0, \ \text{Var}(y_t) = \text{Var}(\epsilon_t) = 1, \ \forall \  t \in \{1, ..., 100\}.
  \]\\
  Say we generate more observations under the same random process, this
  will give us still a value that will be centered on 0, with variance
  1, e.g.~\(\mathbb{E}(y_{150}) = 0, \ \text{Var}(y_{150}) = 1\).\\
\item
  In the centre time series, the series is generated as: \[
      y_t = \epsilon_t + 0.1 \cdot t , \ \epsilon_t \sim \mathcal{N}(0, 1).
  \]This creates a time series with a linear upward trend. Similarly to
  what done before: \[
      \mathbb{E}(y_t) = \mathbb{E}(\epsilon_t) + \mathbb{E}(0.1 \cdot t) = 0.1 \cdot t. 
  \]

  Again, saying that we wish to predict the behaviour of the time series
  at time 150, we know this will be centered on
  \(\mathbb{E}(y_{150}) = 1.5\) (and with which variance?).
\item
  In the rightmost example, the time series was generated for the first
  half of the observations as in A., however after \(t = 50\), a sudden
  shift occurs. Mathematically: \[
  y_t = \begin{cases} 
      \epsilon_t & \text{for } t \leq 50 \\
      \epsilon_t + 5 & \text{for } t > 50
      \end{cases}, \quad \epsilon_t \sim \mathcal{N}(0, 1)
  \] This abrupt change at \(t = 50\) introduces a piecewise structure
  to the data, where the data is seen following a distribution prior to
  the change, \(y_t \sim N(0, 1)\) up to a certain time point \(t=50\),
  and \(y_t \sim N(5, 1)\) after. What can we say about time \(t=150\)?
  Well, to make assumptions we have to assume that other changes are
  happening. However, it is far more interesting to find out where this
  change happened.\\
  In many examples of this module, we will be studying processes that
  are piecewise stationary in the mean and variance, as in this example.
\end{enumerate}

\textbf{Stationarity in the mean and variance.} A time series is said to
be \emph{stationary} in mean and variance, if its mean and variance are
constant over time. That is, for a time series \(y_{1:n}\): \[
    \mathbb{E}(y_t) = \mu \quad \text{and} \quad \text{Var}(y_t) = \sigma^2 \quad \forall \in \{1, ..., n\}
\]

Similarly, a time series is \emph{non-stationary} in the mean and
variance if those change over time.

\textbf{Piecewise stationary in the mean and variance.} A
\emph{piecewise stationary} time series is a special case of a
non-stationary time series. We will say that a time series is
\emph{piecewise stationary} in mean and variance if it is stationary
within certain segments but has changes in the mean or variance at
certain points, known as \emph{changepoints}. After each changepoint,
the series may have a different mean, variance, or both.

\emph{Back to our example.}
...
\item
  In C, \(E[y_t] = E[\epsilon_t] = 0\) for \(t \leq 50\), and
  \(E[y_t] = E[\epsilon_t] + E[3] = 5\) for \(t > 50\). The series is
  therefore piecewise stationary in the mean.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Spectroscopy data.} Changepoint detection is useful in
  spectroscopy data to segment time series of electron emissions into
  regions of approximately constant intensity, accounting for
  large-scale fluctuations in laser power and beam pointing.

  \begin{figure}[H]

  {\centering \includegraphics{source_imgs/photon_emission.png}

  }

  \caption{Electron emission spectroscopy data, Frick, K., Munk, A., \&
  Sieling, H. (2014).}
...
Depending on the model, we could seek for different types of changes in
the structure of a time series. Some of the most common types of changes
include shifts in mean, variance, and trends in regression. For example,
the CPU example above exihibited, in addition to some extreme
observations, both changes in mean and variance.
...
CUSUM statistic is to systematically compare the mean of the data to the
left and right of each possible changepoint \(\tau\). By doing so, we
can assess whether there is evidence of a significant change in the mean
at a given point.

\subsection{The CUSUM statistics}\label{the-cusum-statistics}

The CUSUM statistic compares, for a fixed \(\tau \in \{1, \dots, n-1\}\)
...
our statistics is the absolute value of normal random variable that has
variance \(\sigma^2\). If there is no change at \(\tau\), this
difference is going to be distributed as a standard normal.
...
any point \(\tau\) should be similar. However, if there is a
large-enough change in the mean, the means will differ significantly,
highlighting the changepoint.
...
(in fact it is often chosen as in hypothesis testing).
...
The natural extension of the CUSUM to this situation is to use as a test
statistic the maximum of \(C_\tau\) as we vary \(\tau\):

\[
C^2_{max} = \max_{\tau \in \{1,\ldots,n-1\}} C_\tau^2 / \sigma^2.
\]

And detect a changepoint if \(C^2_{max} > c\) for some suitably chosen
threshold \(c\). The choice of \(c\) will determine the significance
level of the test (we'll discuss this in more detail later).
Graphically, the test will look as follows:

\includegraphics{1_intro_cusum_files/figure-pdf/unnamed-chunk-9-1.pdf}

If we detect a changepoint (i.e., if \(C^2_{max} > c\)), we can estimate
its location by:
...
possible changepoints are: \(\tau = 1, 2, 3\).
...
Statistic}\label{sec-algorithmic-formulation-of-the-cusum-statistic}
...
However, there's an algorithmic trick: by sequentially computing partial
sums, e.g.~\(S_n = \sum_{i=1}^n y_i\), we can shorten out our
computations significantly. In this way we can compute the value of the
means directly as we iterate in the for cycle.
...
figure out why?\\
\strut ~~\(C^2_t \leftarrow \frac{t(n-t)}{n} (\bar{y}_{1:t} - \bar{y}_{(t+1):n})^2\)\\
\strut ~~\textbf{IF} \(C^2_t > C_{max}\)\\
\strut ~~~~\(C_{max} \leftarrow C_t\)\\
\strut ~~~~\(\hat{\tau} \leftarrow t\)\\
\strut \\
\textbf{IF} \(C_{max} / \sigma^2 > c\)\\
...
We can see how the cusum behaves across different values of
\(\tau = 1, \dots, n-1\) in the example below:
...
\subsection{Code the CUSUM algorithm for a unknown change location,
based on the pseudocode
above.}\label{code-the-cusum-algorithm-for-a-unknown-change-location-based-on-the-pseudocode-above.}
...
\def\labelenumi{\alph{enumi}.}
\item
  \(y_t = y_{t - 1} + \epsilon_t, \quad \ t = 2, \dots, n, y_1 = 0, \epsilon_{t} \sim N(0, 1)\).
...
\item
  ...
  \item
    ...
    \(\bar{y}_{1:\tau} - \bar{y}_{(\tau+1):n}\)
  \item
    Conclude that if you standardise the sum, this follows a standard
    normal distribution.
  \end{enumerate}
\end{enumerate}
...
\item
  ...
  pseudocode of Section
  Section~\ref{sec-algorithmic-formulation-of-the-cusum-statistic}.
\item
  Modify your function above to output the CUSUM statistics over all
  ranges of tau.
\item
  Recreate the ``CUSUM Statistics over time'' plot for the Simpsons data
  above.
\end{enumerate}
...
If \(z_1, \cdots, z_k\) are independent, standard Normal random
variables, then:
...
where \(\chi^2_k\) is a chi-squared distribution with \(k\) degrees of
freedom. The chi-squared distribution is a continuous probability
distribution that models the sum of squares of k independent standard
normal random variables: we have met the chi-squared distribution
already in hypothesis testing and constructing confidence intervals. The
shape of the distribution depends on its degrees of freedom. For
\(k=1\), it's highly skewed, but as \(k\) increases, it becomes more
...
If we take the example of last week, and remove the changepoint, we can
observe that the cusum statistics stays constant, and relatively small:
...
\def\labelenumi{\arabic{enumi}.}
\item
  So far, we only studied the behaviour of the statistics for one fixed
  \(\tau\), however, when comparing the maximums, the values of
  \(C_\tau\) are in fact not independent across different \(\tau\)s.
\item
...
\begin{equation}\phantomsection\label{eq-cusum_control}{
...
\(C_\tau^2/\sigma^2\) should increase with \(n\) at a rate of
approximately \(2 \log \log n\). Given that this is a fairly slow rate
of convergence, this suggests that the threshold suggested by this
asymptotic distribution can be conservative in practice, potentially
leading to detect less changepoints than what actually exist.
...
can be done via the following process:
...
\item
  Calculate the test statistic \(C_\tau^2/\sigma^2\) for each one of the
  replicates.
\item
  Set the threshold to be the \((1-\alpha)\) percentile of the
  distribution of the test statistics from simulated data.
\end{enumerate}

This leads to have less conservative thresholds.

\textbf{Theoretical vs Empirical Thresholds} The figure below shows, for
various levels of \(\alpha = 0.01, 0.05, 0.1\), thresholds \(c_\alpha\)
computed from the theoretical distribution of
Equation~\ref{eq-cusum_control} against the Monte Carlo thresholds
obtained from empirical simulations under the null.

\includegraphics{source_imgs/empirical_thres_comparison.png}

We will see how to compute in practice the theoretical and empirical
thresholds in the Lab!
...
As a first example, we show how the CUSUM statistics is nothing but a
specific case of the GLR. To see this, we start from our piecewise
costant signal, plus noise,
\(x_i = f_i + \epsilon_i, \quad i = 1, \dots, n\). Under this model our
data, a linear combination of a Gaussian, is distributed as:
...
f(y\_t \textbar{} \theta) = \frac{1}{\sqrt{2\pi \sigma^2}}
\exp\{-\frac{1}{2 \sigma^2} (y\_t - \mu)\^{}2\}.

\$\$

Therefore, to obtain the likelihood ratio test statistic, we plug our
Gaussian p.d.f. into the LR above, and take the logarithm:
...
\strut ~~~~\(\hat{\tau} \leftarrow \underset{\tau \in \{s, \dots, t-1\}}{\text{arg}\min} \left[ \mathcal{L}(y_{s:\tau}) + \mathcal{L}(y_{\tau+1:t}) - \mathcal{L}(y_{s:t}) \right]\)\\
...
\item
  ...
  \(\beta = 5\) calculate, pen on paper (and calculator), all the
  Optimal Partitioning and Binary Segmentation steps. \textbf{TIP}: To
  speed up computations, you want to pre-compute all segment costs
...
\tightlist
\item
  A vector \texttt{y}, our observations
\item
  A double \texttt{penalty}, corresponding to the \(\beta\) penalty of
  our penalised cost
\item
  A function \texttt{COST}. This function should take as input arguments
  \texttt{y,\ s,\ t}, and act as \(\mathcal{L}(y_{s:t})\). You fill find
  a skeleton below.
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{OP }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{ (y, penalty, COST) \{}

  \DocumentationTok{\#\#\# pre{-}compute all the costs here}

  \DocumentationTok{\#\#\# your initialization here}
  
  \ControlFlowTok{for}\NormalTok{ (t }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\NormalTok{n) \{}
    
    \DocumentationTok{\#\#\# your recursion here}

\NormalTok{  \}}
  

  \FunctionTok{return}\NormalTok{(changepoints)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

For this exercise, we are implementing the Gaussian change-in-mean case.
Therefore, a skeleton for our cost function will be:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{costMean }\OtherTok{=} \ControlFlowTok{function}\NormalTok{(y, s, t) \{}
  
  \CommentTok{\# code for computing the Gaussian change{-}in{-}mean cost here}

  \FunctionTok{return}\NormalTok{(your\_cost)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

You should be then able to call your \texttt{OP} function as:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\NormalTok{y }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\FunctionTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{, }\DecValTok{5}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{, }\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{))}
\FunctionTok{OP}\NormalTok{(y, }\DecValTok{15}\NormalTok{, costMean)}
\end{Highlighting}
\end{Shaded}
...
exists. When \(\kappa\) is equal to the penalty \(\beta\), this gives us
a computational trick to improve on the efficiency. The PELT algorithm
-- acronym for Pruned Exact Linear Time -- (Killick, Fearnhead, and
Eckley (2012)) solves exactly the penalised minimization of
Equation~\ref{eq-optimal-partitioning} with an expected computational
...
The intuition, is that we would prune at every change detected. And if
the changes increase linearly with the length of the data, this means
that our algorithm will achieve a \(\mathcal{O}(n \log n)\)
computational complexity, without any drawbacks!
...
\strut ~~~~~\(cp_t \leftarrow (cp_{\hat\tau}, \hat\tau)\) // Append the
changepoint to the list at the last optimal point\\
\strut ~~~~~\(R_{t+1} \leftarrow \{\tau \in \{R_t \cup \{t\}\} : \mathcal{Q}_\tau + \mathcal{L}(y_{\tau + 1:t}) \leq \mathcal{Q}_t \}\)
...
We first start by drawing the set of intervals
\(\mathcal{F} = \{ [s_1, t_1], \dots, [s_M, t_M] \}\) where:

\[
s_m \sim \text{U}(1, n), \ t_m \sim \text{U}(s_m+1, n)
\]

Then, WBS will have just a couple of alterations to the original Binary
Segmentation:
...
length 1 or less\\
\strut \\
\(\mathcal{M}_{s,e} =\) Set of those indices m for which
\([s_m, t_m] \in \mathcal{F}\) is such that
\([s_m, t_m] \subset [s, t]\).\\
...
\includegraphics{4_algos_and_penalties_files/figure-pdf/unnamed-chunk-9-1.pdf}
...
\includegraphics{4_algos_and_penalties_files/figure-pdf/unnamed-chunk-10-1.pdf}

\begin{enumerate}
\def\labelenumi{\alph{enumi}.}
\item
  Can you identify the elements where there is dependency across
  segments? Once you've done that, rewrite the model for one change by
  seting \(\tau_{i} = 0\). Would you be able to use PELT pruning with
  this one?
\item
...
\includegraphics{4_algos_and_penalties_files/figure-pdf/unnamed-chunk-12-1.pdf}
...
data. Each sequence will have one of the following Gaussian
change-in-mean patterns:\\
\strut \\
\includegraphics{source_imgs/scenarios.png}

The code below will generate 400 sequences, which will be stored in a
list called \texttt{full\_seqs}. Every 100 sequences you will have a
different change-pattern, across the four different change patterns.
...
\item
  ...
  superimposed signals. You should replicate the plot above.
\item
...
the MSE computation above, we can use:
...
\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-3-1.pdf}
...
\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-8-1.pdf}

With the diagnostics:

\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-9-1.pdf}

\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-9-2.pdf}

\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-9-3.pdf}
...
\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-10-1.pdf}
...
\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-12-1.pdf}
...
\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-13-1.pdf}
...
\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-14-1.pdf}
...
\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-15-1.pdf}

\section{Exercises}\label{exercises-4}

\subsection{Workshop exercises}\label{workshop-exercises}

Provide an interpretation of the residuals diagnostics from the Simpsons
dataset:

\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-16-1.pdf}

\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-16-2.pdf}

\includegraphics{5_real_data_files/figure-pdf/unnamed-chunk-16-3.pdf}
