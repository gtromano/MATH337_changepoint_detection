...
1.    For the vector $ y_{1:4} = \{0.5, -0.1, 12.1, 12.4\}$, and a penalty $\beta = 5$ calculate, pen on paper (and calculator), all the Binary Segmentation, and Optimal Partitioning steps. **TIP**: To speed up computations, you want to pre-compute all segment costs $\mathcal{L}(y_{l:u})$.
