...
$$
\frac{C_{\tau}^2 }{\sigma^2} > \lambda,
$$
where the $\lambda \in \mathbb{R}^+$ is a suitable chosen threshold value (similar to a critical value in hypothesis testing!).
...
And detect a changepoint if $C_{max} > c$ for some suitably chosen threshold $c$. The choice of $c$ will determine the significance level of the test (we'll discuss this in more detail later).
...
| **INPUT:** Time series $y = (y_1, ..., y_n)$, threshold $c$
...
Running the CUSUM test on our Simpsons episode results in: 
```{r echo = F}
LR <- function(x) {
...
1.    Compute the mean and variance of the absolute value of difference between the two pre-change and post-change sample means when there is no change, and show that when multiplied by the normalizing constant $1/\sigma^2\sqrt{\frac{\tau(n-\tau)}{n}}$ this follows a standard normal distribution.
