...
$$ where the $c \in \mathbb{R}^+$ is a suitable chosen threshold value (in fact it is often chosen as in hypothesis testing). The $\frac{C_{\tau}^2 }{\sigma^2}$ is known as the CUSUM statistics for a fixed change at $\tau$.
