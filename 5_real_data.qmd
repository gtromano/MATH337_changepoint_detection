...
#### 3. **Residuals over Time Plot**

Finally, this plot shows the residuals on the y-axis against time, with the additional change locations. It is particularly useful for checking if there are patterns in the residuals that suggest issues with the model fit. In the change-in-mean case, we would like to see a random scatter of points around zero over time (as we subtracted the mean from each segment!), with no discernible pattern, and roughly equal spread across all segments (suggesting homoschedasticity).

```{r}
plot(residuals(out), xlab = "t", ylab="residuals",  main = "Residuals over time", pch=16)
abline(v=cpts(out), col = adjustcolor("red", alpha = 0.3))
```

-   **What to Watch for:**
    -   We need to look out for extreme values, as well as a sequence of points centered around 0! These could be segments which only have one or few observations in it, which could be a sign of overfitting. **Note that these will not show up in the two plots above!**
    -   Heteroscedasticity (Non-constant variance): If the residuals' spread increases or decreases across segment, it may indicate heteroscedasticity, which violates one of the key assumptions of the change-in-mean model (constant variance of residuals). If you observe heteroschedasticity only in one of the segments, it might mean that we are underestimating the number of the changes!
...
```{r echo=FALSE}
hist(residuals(out), main = "Histogram of the residuals", breaks = 30)
qqnorm(residuals(out), main = "Normal Q-Q Plot of the Residuals")
qqline(residuals(out), col = "red")
plot(residuals(out), xlab = "t", ylab="residuals",  main = "Residuals over time", pch=16)
abline(v=cpts(out), col = adjustcolor("red", alpha = 0.3))
...
The residuals over time gives us a better figure and provides the strongest evidence of overfitting: we can see that we have a lot of "extreme" residuals followed by a streak of 0s... The extreme ones come from segments over large variance, the 0-centered ones, come from single point segments. This is a clear sign of heteroschedasticity, where we overfitted across the sequence with larger variance.
