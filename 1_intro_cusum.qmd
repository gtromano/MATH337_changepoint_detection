...
Time series can be classified into different categories based on their statistical properties over time. For example, let us look at this example below:
...
**Stationarity in the mean and variance.** For the scope of this Section, and in many examples of this module, we will be talking about stationarity in the mean and variance of a process.

1.  **Stationary Time Series**: A time series is said to be *stationary* in mean and variance, if its mean and variance are constant over time.
...
|   $C_t \leftarrow \frac{t(n-t)}{n} (\bar{y}_{1:t} - \bar{y}_{(t+1):n})^2$
|   **IF** $C_t > C_{max}$
...
a.  $y_t = y_{t - 1} + \epsilon_t, \quad \ t = 2, \dots, n, y_1 = 0, \epsilon_{1:n} \sim N(0, 1)$. This is a random walk model. Let's start by computing the expected value and variance of $y_t$ across all $t$. **TIP:** Start by expanding $y_{t}$ in terms of the noise components...
